...
function isSmartTarget(relativePath: string): boolean {
  ...
  if (lowercasePath.includes(".vscode/") || lowercasePath.includes(".idea/"))
    return true;
  if (filename === ".cursorrules" || filename === ".windsurfrules") return true;

  // 2. AI/LLM Rules
  if (
    lowercasePath.includes("prompts/") ||
  ...
  if (
    ...
    return true;
  if (filename === "Jenkinsfile" || filename === "Makefile") return true;
  ...
  if (
    ...
    return true;
  if (filename.startsWith(".env") || filename === ".npmrc") return true;

  return false;
}

// Optimized recursive scanner
function getSmartFiles(
  dirPath: string,
  rootPath: string,
  arrayOfFiles: string[] = [],
): string[] {
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider("absSidebar", sidebarProvider),
  );
  ...
  let disposable = vscode.commands.registerCommand(
    ...
    async () => {
      ...
      channel.appendLine(
        `[ABS Core] Starting Smart Scan at ${new Date().toISOString()}`,
      );
      channel.appendLine(`[INFO] Root: ${workspaceRoot}`);
      channel.appendLine(
        `[INFO] Strategy: Prioritizing Config/IDE/AI Files (.cursorrules, .env, .vscode, etc)`,
      );
        ...
        async () => {
          try {
            ...
            if (allFiles.length > MAX_FILES) {
              channel.appendLine(
                `[WARN] Found ${allFiles.length} files. Optimizing to top ${MAX_FILES} high-value targets.`,
              );
              finalFilesToScan = allFiles.slice(0, MAX_FILES);
            } else {
              channel.appendLine(
                `[INFO] Total target files found: ${finalFilesToScan.length}`,
              );
            }
            ...
            for (const fileToScan of finalFilesToScan) {
              ...
              try {
                ...
                channel.appendLine(
                  `[ERROR] Could not read ${path.basename(fileToScan)}: ${err}`,
                );
              ...
              channel.appendLine(
                `[SCAN] Analyzing: ${path.basename(fileToScan)}`,
              );

              try {
                const config = vscode.workspace.getConfiguration("abs");
                const apiUrl = config.get<string>("apiUrl") || ABS_API_URL;
                const apiKey = config.get<string>("apiKey") || "sk-producer-v0";
                ...
                if (response.ok) {
                  ...
                  if (result.decision === "DENY" || result.allowed === false) {
                    channel.appendLine(
                      `[VIOLATION] ${path.basename(fileToScan)}: DENIED`,
                    );
                  ...
                  channel.appendLine(
                    `[WARN] API returned ${response.status} for ${path.basename(fileToScan)}`,
                  );
                ...
                channel.appendLine(
                  `[ERROR] API Call failed for ${path.basename(fileToScan)}: ${apiErr}`,
                );
            ...
            if (issuesFound > 0) {
              vscode.window.showWarningMessage(
                `ABS: Found ${issuesFound} issues. Report saved to ABS_AUDIT.md`,
              );
              channel.appendLine(
                `[RESULT] Scan complete. ${issuesFound} issues found.`,
              );
            } else {
              vscode.window.showInformationMessage(
                "ABS: Scan complete. Clean. Report saved to ABS_AUDIT.md",
              );
