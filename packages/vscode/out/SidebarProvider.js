        ...
        webviewView.webview.onDidReceiveMessage(async (data) => {
            switch (data.type) {
                ...
                case 'onScanLive': {
                    ...
                    const apiUrl = config.get('apiUrl') || 'https://api.abscore.app';
                    vscode.window.withProgress({
                        ...
                        try {
                            ...
                            if (response.ok) {
                                ...
                                vscode.window.showInformationMessage(`✅ ABS Server OK: ${data.status || 'healthy'}`);
                            }
                            else {
                                vscode.window.showWarningMessage(`⚠️ ABS Server returned ${response.status}`);
                            }
                        }
                        catch (err) {
                            vscode.window.showErrorMessage(`❌ Could not reach ABS Server: ${err}`);
                        }
                    });
                    break;
                }
