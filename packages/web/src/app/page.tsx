...
import { Terminal, Shield, Zap, Lock, Github, Check, X, ArrowRight } from "lucide-react";
    ...
    <div className="min-h-screen bg-black text-white font-sans selection:bg-orange-500 selection:text-white">
      ...
      <nav className="border-b border-zinc-800 bg-zinc-950/50 backdrop-blur fixed w-full z-50">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-6 text-sm font-medium">
            <a href="https://github.com/eusheriff/abs-core" className="hover:text-orange-500 transition-colors flex items-center gap-2">
              <Github className="w-4 h-4" /> GitHub
            </a>
            <a href="https://oconnector.tech/abs" className="bg-white text-black px-4 py-2 rounded-full hover:bg-zinc-200 transition-colors">
              Get Enterprise
            </a>
          </div>
        </div>
      ...
      <section className="pt-32 pb-20 px-6 max-w-6xl mx-auto text-center">
        ...
        <div className="flex flex-col md:flex-row items-center justify-center gap-4">
          <div className="flex items-center gap-0 rounded-lg bg-zinc-900 border border-zinc-800 p-1 pl-4 pr-1">
            <span className="text-zinc-400 font-mono text-sm mr-4">$ npm install @abs/scan</span>
            <button className="bg-zinc-800 hover:bg-zinc-700 text-white p-2 rounded transition-colors" title="Copy">
              <Terminal className="w-4 h-4" />
            </button>
          </div>
          <a href="#compare" className="flex items-center gap-2 text-zinc-400 hover:text-white transition-colors text-sm font-medium">
      ...
      <section className="px-6 max-w-5xl mx-auto mb-32">
        <div className="rounded-xl border border-zinc-800 bg-zinc-950/50 overflow-hidden shadow-2xl shadow-orange-500/10">
          <div className="flex items-center gap-2 px-4 py-3 border-b border-zinc-800 bg-zinc-900/50">
            ...
            <span className="ml-2 text-xs text-zinc-500 font-mono">scanner-demo.ts</span>
          </div>
          <div className="p-6 font-mono text-sm overflow-x-auto">
            <div className="text-zinc-400">import <span className="text-white">{`{ ABS }`}</span> from <span className="text-green-400">'@abs/scan'</span>;</div>
            <div className="h-4" />
            <div className="text-zinc-500">// 1. Initialize local scanner</div>
            <div className="text-zinc-400">const abs = new <span className="text-orange-400">ABS</span>({`{ mode: 'scanner' }`});</div>
            <div className="h-4" />
            <div className="text-zinc-500">// 2. Log AI interactions (Non-blocking)</div>
            <div className="text-purple-400">await <span className="text-blue-400">abs.log</span>({`{`}</div>
            <div className="text-zinc-300 pl-4">input: <span className="text-green-400">"Delete production database"</span>,</div>
            <div className="text-zinc-300 pl-4">output: <span className="text-green-400">"Executing DELETE schema..."</span>,</div>
            <div className="text-zinc-300 pl-4">policy: <span className="text-green-400">"strict-safety"</span></div>
            <div className="text-purple-400">{'}'});</div>
            <div className="h-4" />
            <div className="text-zinc-500">// Output: [ABS Scanner 🛡️] Policy Violation Detected!</div>
          </div>
        </div>
      </section>
      ...
      <section id="compare" className="px-6 max-w-6xl mx-auto mb-32">
        <div className="text-center mb-16">
          ...
          <p className="text-zinc-400">Start with visibility, upgrade to enforcement.</p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <div className="rounded-2xl border border-orange-500/30 bg-orange-500/5 p-8 relative overflow-hidden">
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h3 className="text-xl font-bold text-white">Enterprise Runtime</h3>
                <p className="text-sm text-orange-200">Production Enforcement</p>
              </div>
            </div>
            <div className="space-y-4 mb-8">
            ...
            <div className="mt-auto">
              ...
              <p className="text-xs text-orange-200/60 mb-6">Volume-based licensing</p>
              <a href="https://oconnector.tech/abs" className="block w-full py-3 rounded-lg bg-orange-600 hover:bg-orange-500 transition-colors font-medium text-center">
                Contact Sales
              </a>
            </div>
          </div>
      ...
      <footer className="border-t border-zinc-900 py-12 text-center text-zinc-500 text-sm">
        <p>&copy; {new Date().getFullYear()} OConnector. All rights reserved.</p>
...
function FeatureItem({ text, check, highlight }: { text: string; check: boolean; highlight?: boolean }) {
  return (
    <div className="flex items-center gap-3">
      {check ? (
        <Check className={`w-5 h-5 ${highlight ? 'text-orange-500' : 'text-zinc-500'}`} />
      ) : (
