 ...
 */
export const FREE_TIER_POLICIES: Policy[] = [
    {
        ...
        evaluate: (proposal, event) => {
            ...
            if (/\b(?:\d[ -]*?){13,16}\b/.test(payload)) return 'DENY';
            // Simple Regex for CPF
            if (/\b\d{3}\.\d{3}\.\d{3}-\d{2}\b/.test(payload)) return 'DENY';
            return 'ALLOW';
        }
    },
    {
        ...
        evaluate: (proposal, event) => {
            ...
            if (payload.includes('password') || payload.includes('secret_key')) return 'DENY';
            return 'ALLOW';
        }
    }
];
 ...
 */
export const RUNTIME_PAID_POLICIES: Policy[] = [
    {
        ...
        evaluate: (proposal, event) => {
            ...
            if (event.payload && (event.payload as any).amount > 10000) {
                return {
