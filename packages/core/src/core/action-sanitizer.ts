...
export interface ActionSanitizeResult {
  canSanitize: boolean;
...
export interface ActionSanitizeRule {
  name: string;
  eventType: string | RegExp;
  check: (payload: Record<string, unknown>) => boolean;
  sanitize: (payload: Record<string, unknown>) => ActionSanitizeResult;
}

// Built-in sanitization rules
const ACTION_SANITIZE_RULES: ActionSanitizeRule[] = [
  {
    name: 'sql-add-limit',
    ...
    check: (payload) => {
      ...
      ) && !query.includes('LIMIT') && !query.includes('WHERE');
    },
    sanitize: (payload) => {
...
export class ActionSanitizer {
  private rules: ActionSanitizeRule[];

  constructor(customRules: ActionSanitizeRule[] = []) {
   ...
   */
  trySanitize(eventType: string, payload: Record<string, unknown>): ActionSanitizeResult | null {
   ...
   * Add a custom rule
   */
  addRule(rule: ActionSanitizeRule): void {
   ...
   * Get all rules
   */
  getRules(): ActionSanitizeRule[] {
    return [...this.rules];
  }
}

// Singleton instance
let instance: ActionSanitizer | null = null;

export function getActionSanitizer(customRules?: ActionSanitizeRule[]): ActionSanitizer {
  if (!instance) {
